...
(function(){
	...
	deleteFolderRecursive(config.root);
...
function fetchFile(oUrl,tIns,callback){
	...
	if (!specFlag.exclude && (specFlag.include || isView || oUrlObj.host in config.resource)){
		...
		*/
		if(hostname){
			var filePath=path.resolve(root,"../",hostname,oUrlObj.pathname.replace(/^\//,'').replace(/[^\w\.\/\\]/g,'_'))+(isView?'.html':'');
		...
		var opts=url.parse((tUrl.indexOf('web/lizard.seed.js')!=-1)?tUrl.replace('web/lizard.seed.js','hybrid/lizard.seed.src.js'):tUrl);
